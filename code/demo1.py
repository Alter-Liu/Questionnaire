...
# 感谢上方博客作者提供的代码，本代码针对以上代码进行了部分修改，以适应现在的问卷星 ————2023.07.23
# 1.增加了指定填写份数的功能
# 2.使用了random。random（）函数和random。sample（），简化了随机产生选项的过程，尤其是多选结果的产生，同时大家可以按需调节函数，以得到需要的结果
# 3.针对现在的验证方式，滑块几乎没有出现过，但是在填写过程中会随机出现一个弹窗，对此弹窗，本文进行了对应的处理以应对现在的验证方式
# 4.由于现在验证不是固定的模式，所以将“确认提交”、“弹窗确认”、“点击验证”和“滑块验证”都放入了try中以保证脚本运行的连续性
# 5.对于填空题的代码，保留在单选题点击代码段的注释中，如有需要，可以自行选用
import random
import sys
